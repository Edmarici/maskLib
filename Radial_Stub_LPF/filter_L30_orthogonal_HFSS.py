...
extraction) for the L30 orthogonal (perpendicular-fan, 13mm chip) variant
(Radial_Stub_LPF/filter_L30_orthogonal.py ->
DXF/filter_L30_orthogonal_CHIP_L30_ORTHO.gds).

Sibling of filter_L30_HFSS.py - built to run the exact same wave-port S21
pipeline against filter_L30_orthogonal.py's geometry, so the two results are
directly comparable (see notebooks/L30_orthogonal_design_notes.md). Every
low-level pyEPR 0.8 API workaround, the Unite() winding-order fix, and the
wave-port restructuring are unchanged copies of filter_L30_HFSS.py's own
(hand-verified against the real installed pyEPR/AEDT this session) - see
that file's own docstring for the full rationale on each point. Only the
project/output naming and the geometry-source import below differ.

RUN VIA THE SEPARATE PYEPR ENVIRONMENT, NOT THIS REPO'S OWN .venv:
    C:\\Users\\epm114\\.AnE\\Scripts\\python.exe filter_L30_orthogonal_HFSS.py
...
WHAT'S DIFFERENT FROM filter_L30_HFSS.py:
  ...
  GEOMETRY SOURCE - imports regenerate_metal_pieces/fan_arc_points from
    verify_L30_orthogonal_hfss_export instead, which replays
    filter_L30_orthogonal.py's tables (all tilt=0.0, 13000um-wide chip)
    instead of filter_L30.py's.

  LARGER CROPPED MODEL DOMAIN - the live metal bounding box here is wider
    (~8890um transverse vs filter_L30.py's tilted/compact footprint), so
    the cropped Substrate/Package region (metal bbox + MODEL_PADDING_UM/
    PACKAGE_PADDING_UM, unchanged constants) comes out proportionally
    wider too - no code change needed, this scales automatically (see
    filter_L30_HFSS.py's own CROPPED MODEL DOMAIN docstring section).

  STALK_FAN_OVERLAP_UM re-check - filter_L30_HFSS.py's value (5.0um) was
    empirically tuned against TILTED stalk/fan joints. This file's joints
    are all perpendicular instead - kept the same starting value, but this
    is exactly the kind of thing the RUN_ANALYSIS=False smoke test below
    exists to catch (a bad Unite() would show up as a null/Unclassified
    body or a "invalid parameters to Unite operation" error at that step).

  PROJECT/OUTPUT NAMING ONLY - project_name, and the S21/S11 CSV/PNG
    basenames, are renamed to filter_L30_orthogonal_* so results don't
    collide with filter_L30.py's own HFSS/filter_L30_S21.csv etc.
...
from verify_L30_orthogonal_hfss_export import regenerate_metal_pieces, fan_arc_points  # noqa: E402
...
# Smoke test (RUN_ANALYSIS=False) passed: Unite() succeeded cleanly on the
# perpendicular-joint geometry (confirmed directly against the live AEDT
# session - GetObjectsInGroup("Unclassified") empty, PinPad/Substrate/Package
# the only 3 solids, PinPad material=aluminum). Flipped to True per Eddie's
...
project_name = 'filter_L30_orthogonal_S21'
...
# the full Unite()-winding-order story; draw_fan_native/STALK_FAN_OVERLAP_UM
# below are unchanged copies of that fix.
# ===============================================================================
...
# See module docstring "STALK_FAN_OVERLAP_UM re-check" - same starting value
# as filter_L30_HFSS.py, worth re-verifying against this file's perpendicular
# joints via the RUN_ANALYSIS=False smoke test below.
STALK_FAN_OVERLAP_UM = 5.0

geom = regenerate_metal_pieces(stalk_fan_overlap_um=STALK_FAN_OVERLAP_UM)
pieces = geom['pieces']
print('Regenerated %d metal piece(s) directly from filter_L30_orthogonal.py (no DXF/GDS round-trip)' % len(pieces))

metal_names = []
for i, (label, hull, fan_params) in enumerate(pieces):
    if fan_params is not None:
        sheet = draw_fan_native(model, label, fan_params)
    else:
    ...
    print('  built solid %d/%d: %s (%s)' % (i + 1, len(pieces), sheet, 'arc' if fan_params else '%d vertices' % len(hull)))
...
all_xs = [x for _label, hull, _fan_params in pieces for x, y in hull]
all_ys = [y for _label, hull, _fan_params in pieces for x, y in hull]
...
if not RUN_ANALYSIS:
    print('Setup complete (RUN_ANALYSIS=False). Inspect the model in AEDT '
          ...
          'solid, not null/Unclassified - this is the main thing worth '
          're-checking since this geometry\'s stalk/fan joints are '
          'perpendicular instead of tilted), then set RUN_ANALYSIS=True and '
          're-run to solve + pull S21/S11.')
else:
    ...
    # pyEPR's HfssFrequencySweep.get_report_arrays()/HfssReport.get_arrays()
    # export to a path from tempfile.mktemp() and immediately read it back -
    # this failed with FileNotFoundError the first time this script ran for
    # real (ExportToFile's COM call returned without error, but AEDT never
    # actually wrote the file at that particular temp path - a real solve
    # with real solution data existed the whole time, confirmed by manually
    # re-exporting to an explicit path in this repo's own HFSS/ folder,
    # which worked immediately). Bypass the buggy default here: call
    # create_report() (same as get_report_arrays does internally) but
    # export to our own explicit, known-writable path instead of relying on
    # tempfile.mktemp().
    ...
    s21_csv_path = os.path.join(out_dir, 'filter_L30_orthogonal_S21.csv')
    ...
    s11_csv_path = os.path.join(out_dir, 'filter_L30_orthogonal_S11.csv')
    ...
    try:
        ...
        ax.set_ylabel('dB')
        ax.set_title('filter_L30_orthogonal - S21/S11 (PLACEHOLDER geometry, proxy ground)')
        ax.legend()
        ax.grid(True, alpha=0.3)
        png_path = os.path.join(out_dir, 'filter_L30_orthogonal_S21.png')
